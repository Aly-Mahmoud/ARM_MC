...
NVIC_Error_Status NVIC_GetPendingIRQ        (IRQ_ID_t IRQn, U8 *Pending_State)
{
    NVIC_Error_Status Local_Error_Status = NVIC_NOK;
    ...
    if ( IRQn & BIT14_MASK )
    {
        ...
        /*Get the REG_INDEX*/
            U32 REG_INDEX = IRQn / NVIC_REG_THRESHOLD;
        /*----------------*/

        /*Get the BIT_INDEX*/
            U32 BIT_INDEX = IRQn % NVIC_REG_THRESHOLD;
        /*----------------*/

        /*  Get the suitable bit in the suitable register  */
            *Pending_State = (NVIC->ICPR[REG_INDEX] & (BIT00_MASK << BIT_INDEX))>>BIT_INDEX;
        /*-------------------------------------------------*/
    }
